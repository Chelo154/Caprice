          ...
          <div class="row">
                    ...
                    <div class="row">
                      ...
                      <form method="POST" action="{{route('order.agregarmesa',$order)}}">
                        ...
                        @method('PUT')
                          ...
                          <button type="submit" class="btn btn-primary">Confirmar Mesa  </button>
                        </form>
                          @else
                  ...
                  <form action="{{route('order.agregarproducto',[$order,$producto])}}" method="POST">
                    ...
                    <h5 class="letra-negocio-blanca">Id del Producto: {{$producto->id}}</h5>
                        ...
                        <h5 class="letra-negocio-blanca">Precio Unitario : $ {{$producto->precio}}</h5>                       
                          ...
                          <button type="submit" class="btn btn-primary letra-negocio-blanca">Agregar Producto</button>
                        </form>
                    @endif
                  </div>
                  <hr class="linea-naranja">
            ...
            <hr class="linea-naranja">
             ...
             <form action="{{route('order.registrarcomanda',$order)}}" method="POST">
              @csrf
              @method('PUT')
              <button type="submit" class="btn btn-primary">Confirmar Comanda</button>            
            </form>
            <br>
            <br>
            <form action="" method="POST">
              @csrf
              @method('PUT')
              <button type="submit" class="btn btn-primary">Cancelar Comanda</button>            
            </form>     
            </div>     
